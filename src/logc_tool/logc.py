...
def parse_by_key(file, keys):
    ...
    if len(keys) > 1:  # For filtering logs based on more than one keyword
        ...
        for log in file:
            if keys[
                0] in log:  # As it is a list, but as we know it only has one element, access index zero to use it as the filter
                tmp.append(log.strip())
...
def and_filter(file, keys):
    ...
    for log in file:
        if all([key in log for key in
                keys]):  # Only if ALL keys within the key list are in the log will the log be included in the tmp list
            tmp.append(log.strip())
...
def write_new_file(log_files, destination, file_names):
    with open(pathlib.Path(destination),
              "a") as new_file:  # Open the destination as a new file and in append mode to support more than one file

        for i in range(
                len(log_files)):  # Iterate over a range instead of the file itself to have an index for both the file itself and the file title

            new_file.write(f'Log file: "{file_names[i]}"\n\n')  # Write the title of the current file

            if log_files[
                i] == "Pattern not found":  # In case of just filtering with -t, to prevent this message from displaying in a weird way
